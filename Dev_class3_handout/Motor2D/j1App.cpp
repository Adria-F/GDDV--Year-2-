...
void j1App::DoSave() const
{
	...
	{
		pModule = item->data;
		pModule->Save(node.append_child(pModule->name.GetString()));
	}
...
void j1App::DoLoad()
{
	...
	{	
		pModule = item->data;
		if (node.child(pModule->name.GetString()) != nullptr)
		{
			pModule->Load(node.child(pModule->name.GetString()));
		}
		else
		{
			LOG("No section for module: %s", pModule->name.GetString());
		}
	}
