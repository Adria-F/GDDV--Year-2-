...
bool j1App::Awake()
{
	...
	if (result)
	{
		LOG("Parsed document without errors\n");
		name = (config.child("config").child("name"));
	}
	else
	{
		LOG("Error while parsing document: ");
	...
	while(item != NULL && ret == true)
	{
		...
		if (config.child("config").child(item->data->name.GetString()) != nullptr)
		{
			node = &config.child("config").child(item->data->name.GetString());
		}
