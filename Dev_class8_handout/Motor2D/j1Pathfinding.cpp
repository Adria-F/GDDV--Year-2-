...
int j1PathFinding::CreatePath(const iPoint& origin, const iPoint& destination)
{
	...
	PathList open, close;
	PathNode start(0, 0, origin, nullptr);
	open.list.add(start);
	while (open.list.count() > 0)
	{
		...
		PathList neighbors;
		lowest->data.FindWalkableAdjacents(neighbors);
		p2List_item<PathNode>* curr_neighbor = neighbors.list.start;
		...
		{
			if (close.Find(curr_neighbor->data.pos) == NULL)
			{
				curr_neighbor->data.CalculateF(destination);

				p2List_item<PathNode>* old_instance = open.Find(curr_neighbor->data.pos);
